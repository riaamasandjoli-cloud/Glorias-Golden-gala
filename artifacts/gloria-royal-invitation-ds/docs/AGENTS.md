# Gloria Royal Invitation Design System design system
...
workspace package (`@workspace/gloria-royal-invitation-ds`): other artifacts depend
...
- Source-derived visual language — burgundy wine surfaces, metallic gold emphasis,
  cream text, Cinzel display type, DM Sans utility labels, fine rules, near-square
  framing, and slow ceremonial reveals.
- Source-derived preview guidance — composition, invitation voice, and motion
  rules are documented under Overview, Content, and Motion.
...
`@workspace/gloria-royal-invitation-ds/components/...`. Never use a `@/` alias inside
...
for `@workspace/gloria-royal-invitation-ds` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/gloria-royal-invitation-ds` as a `workspace:*` dependency,
run `pnpm install`, and import directly from this package.

Read only the guides required by the current task:

- Building or styling web UI: `artifacts/gloria-royal-invitation-ds/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/gloria-royal-invitation-ds/docs/consuming-expo.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/gloria-royal-invitation-ds/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/gloria-royal-invitation-ds/docs/migrating-expo.md`
